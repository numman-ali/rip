...
- Hosted web-search controls now live there too:
  - `web_search.enabled`: enable/disable hosted web search
...
- Doctor now does the same for hosted web search:
  - `openresponses.web_search`: the layered web-search request preference RIP resolved from config/env/per-run overrides
  - `openresponses.compat.web_search.effective`: the web-search config RIP will actually send after applying route support/degradation rules (canonical for OpenAI; provider-extension mapped for OpenRouter)
