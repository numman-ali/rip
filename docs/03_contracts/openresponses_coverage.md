...
- `openresponses.hosted_web_search`: hosted web-search request controls, output/citation/source fidelity, and provider/model route diagnostics.
...
- Request-composition proof coverage now explicitly covers:
  ...
  - canonical `web_search` hosted-tool request composition for OpenAI/native routes (`search_context_size`, `external_web_access`, and `user_location` support are modeled from the split schema)
  - provider-prefixed hosted-tool extension validation and OpenRouter `openrouter:web_search` request composition (`search_context_size` maps to `parameters.search_context_size`; `user_location` maps to `parameters.user_location`)
  - stateful follow-up composition with `previous_response_id`
- Canonical hosted web search is now a first-class OpenResponses request family. Config/env/per-run overrides resolve `web_search`, `rip run` exposes enable/disable plus schema-backed context/external-access flags, fullscreen TUI exposes a route-aware enable toggle, server session/thread APIs accept typed overrides, SDK exposes typed controls, and `config.doctor` surfaces requested vs effective web-search support/warnings.
- Live cmux proof now covers the canonical OpenAI path too: `gpt-5.4-nano` with `RIP_OPENRESPONSES_WEB_SEARCH=true` emitted canonical `web_search_call` streaming/completed items, all checked provider events had empty `errors` / `response_errors`, and the fullscreen TUI rendered the hosted-tool cards instead of a provider-error overlay.
- OpenRouter web search is not treated as canonical `web_search`. Current OpenRouter docs point to provider-specific `openrouter:web_search` (and mark the old `plugins` / `:online` paths deprecated), so RIP now maps the high-level hosted-web-search capability to that provider extension only in the OpenRouter compat profile. Documented extension parameters such as `search_context_size` and `user_location` are mapped through `parameters`; unsupported canonical fields such as `external_web_access` are omitted with explicit warnings.
- Provider validation accepts provider-prefixed extension output items such as OpenRouter's `openrouter:web_search` when they satisfy the OpenResponses extension contract (`id`, prefixed `type`, `status`). This is validation-only normalization against a schema-backed hosted-tool item; raw provider payloads remain preserved in `provider_event`, and unprefixed unknown item types still fail.
...
- Provider validation also supports OpenAI-specific validation-only normalization for canonical `response.tools[].type="web_search"` echoes while the vendored OpenResponses response-resource schema still expects the older preview-tool shape. Raw provider payloads remain unchanged in `provider_event`.
...
- Current explicit gap: provider/model profiles may record multimodal/image/file/video, richer provider-extension hosted-tool parameters, MCP rows, and deeper per-value `include` subsets, but RIP does not yet expose the multimodal, MCP, and richer provider-extension request families as first-class operator-configurable OpenResponses request fields. Typed reasoning controls (`effort`, `summary`), typed `include` selection, canonical `web_search` controls, and the OpenRouter `openrouter:web_search` provider-extension bridge are now shipped. OpenAI's current docs mention web-search `filters`, but the vendored OpenResponses split schema does not yet include that field, so it remains a tracked schema-sync/provider-extension follow-up rather than a hand-added request field.
