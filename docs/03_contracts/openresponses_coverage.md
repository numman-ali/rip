...
- Current explicit gap: provider/model profiles may record multimodal/image/file/video, hosted tools, MCP rows, and fine-grained per-value `include` subsets, but RIP does not yet expose the multimodal and hosted-tool/MCP request families as first-class operator-configurable OpenResponses request fields. Typed reasoning controls (`effort`, `summary`) and typed `include` selection are now shipped; deeper provider/model-specific value subsets remain curated in docs/profile state first and must not be described as proven runtime support until they have route-level evidence.
