...
- Current explicit gap: provider/model profiles may record multimodal/image/file/video, hosted tools, MCP rows, and deeper per-value `include` subsets, but RIP does not yet expose the multimodal and hosted-tool/MCP request families as first-class operator-configurable OpenResponses request fields. Typed reasoning controls (`effort`, `summary`) and typed `include` selection are now shipped, and seeded per-value include curation exists for OpenRouter; broader provider/model-specific subsets must still earn route-level evidence before they are described as proven runtime support.
