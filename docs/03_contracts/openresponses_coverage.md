...
- Provider request builder validates CreateResponseBody payloads (errors captured; payload preserved); tool fields use per-variant validation to avoid jsonschema oneOf failures; request sending is wired for prompt-only streaming in `ripd` (tool-call loop still pending).
