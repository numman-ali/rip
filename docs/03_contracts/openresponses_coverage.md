...
- `openresponses.hosted_web_search`: canonical hosted web-search request controls, output/citation/source fidelity, and provider/model route diagnostics.
...
- OpenRouter web search is not treated as canonical `web_search` in this slice. Current OpenRouter docs point to provider-specific `openrouter:web_search` (and mark the old `plugins` path deprecated), so RIP degrades canonical `web_search` on OpenRouter with explicit compat warnings and tracks provider-extension support under `openresponses.extensions`.
...
- Current explicit gap: provider/model profiles may record multimodal/image/file/video, provider-extension hosted tools, MCP rows, and deeper per-value `include` subsets, but RIP does not yet expose the multimodal, MCP, and provider-extension hosted-tool request families as first-class operator-configurable OpenResponses request fields. Typed reasoning controls (`effort`, `summary`), typed `include` selection, and canonical `web_search` controls are now shipped. OpenAI's current docs mention web-search `filters`, but the vendored OpenResponses split schema does not yet include that field, so it remains a tracked schema-sync/provider-extension follow-up rather than a hand-added request field.
