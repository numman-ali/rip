...
- Shipped:
  ...
  - TUI overlay input-ownership pass — non-palette overlays now behave like real overlays instead of leaking keys/mouse through to the canvas underneath. Help/Debug/Activity/detail overlays own arrow/PageUp/PageDown + mouse-wheel scrolling, clicks no longer refocus the hidden canvas behind them, and overlays that can exceed one viewport (`Help`, `Activity`, `Tasks`, tool/task/error details) now render from full content with an explicit overlay scroll offset instead of pre-truncating to the first visible page. The keylight also now tells the truth for thread picker and error recovery instead of showing the generic overlay hint.
  - TUI menu mouse-truth pass — palette rows and thread-picker rows are now actually clickable, clicks outside those overlays dismiss them instead of being swallowed, and the in-product copy now explains the click behavior directly so the command surfaces stop looking mouse-capable while secretly acting keyboard-only.
  - TUI reasoning truth pass — the canvas now ingests canonical OpenResponses `reasoning` items from `response.output_item.{added,done}` / `response.completed`, so providers that populate reasoning directly on the item can surface summaries without depending on the delta-only reasoning events. When a route genuinely used reasoning but returned no visible summary/text, the agent turn now says so explicitly instead of making the request look ignored.
  - TUI stop/cancel truth pass — `Esc` now stops the active session when no overlay owns input, `Ctrl-C` / quit stop the active session by default before leaving the TUI, and the fullscreen server path now drives the real `session.cancel` capability instead of just dropping the client loop. The backend cancel path now emits a terminal `session_ended(cancelled)` event, writes the final snapshot, and appends one `continuity_run_ended(reason=cancelled)` frame for continuity-linked runs.
  - Explicit detach/keep-running v1 — detach is now the intentional second lifecycle path instead of an accidental side effect. `rip run --detach` starts a run and returns linkage without opening the event stream, the fullscreen TUI command palette now exposes `Detach and keep running`, and the TypeScript SDK exposes `runDetached()` across both exec and HTTP transports. Default exit semantics remain stop-first.
- Remaining:
...
- Status (2026-04-19):
  ...
  - Implemented: canonical hosted `web_search` is now first-class through the same provider-boundary compatibility layer. OpenAI/native routes receive schema-backed `tools: [{type:"web_search", ...}]`, OpenRouter receives its documented provider extension `tools: [{type:"openrouter:web_search", parameters:{search_context_size,user_location}}]` when the same high-level hosted-web-search capability is enabled, and CLI/TUI/server/remote/SDK surfaces now expose the request family or a tracked surface-specific subset.
  - Implemented: provider-prefixed hosted-tool extensions are accepted by request validation when the tool `type` uses an implementor slug (`provider:tool`), matching the OpenResponses extension posture while still rejecting unprefixed unknown tools.
  - Implemented: provider-prefixed hosted-tool output items are accepted by stream/response/item validation when they satisfy the OpenResponses extension item contract (`id`, prefixed `type`, `status`), removing false provider errors from successful OpenRouter `openrouter:web_search` runs while preserving raw payloads. Fullscreen TUI now shows hosted web search as both an always-visible `web on` hero chip and a collapsible hosted-tool card.
  - Verified: fresh fullscreen TUI cmux run against OpenAI `gpt-5.4-nano` with canonical `web_search` enabled emitted canonical `web_search_call` items, rendered them as `web_search` cards, and had no hidden `provider_error` / `response_errors` in the event log.
  - Implemented: the resolved compatibility profile is now visible inside the fullscreen TUI without running a separate doctor command. The Models palette chips each configured route from the same provider/model profile used by request composition (`native`/`compat`, `stateful`/`stateless`, reasoning, hosted web-search, image support), and the Options palette includes a non-mutating active-route health row that mirrors `rip config doctor` / SDK `configDoctor()` diagnostics.
  - This is intentionally the start of a broader provider/model health layer, not the end state.
- Ready:
  - Add richer TUI and CLI/SDK controls for web-search `user_location`, future schema-backed `filters`, and OpenRouter-specific `engine` / `max_results` / `max_total_results` / domain filters.
  - Expand from seeded health rows into more surfaced runtime behavior: provider/model profiles should continue to drive SDK diagnostics, always-visible status chrome, and explicit “why this degraded” messages.
  ...
  - Surface the resolved compatibility profile in the remaining operator-facing diagnostics/status surfaces so there is one canonical place to see what each provider/model combination supports, what is normalized, and what still degrades.
