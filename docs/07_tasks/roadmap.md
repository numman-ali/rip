...
- Ready:
  ...
  - Lift the remaining documented-only request knobs into the real runtime/config surface where appropriate: multimodal/image/file inputs, hosted tools, MCP request composition, and `include`-level controls such as `reasoning.encrypted_content`.
