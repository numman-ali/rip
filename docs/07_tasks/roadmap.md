...
- Ready:
  ...
  - Expose first-class reasoning controls on active operator surfaces, not just in config/env plumbing: headless CLI flags, TUI command/options/model surfaces, and surface diagnostics should all be able to set/show `reasoning.effort` + `reasoning.summary` for the active route without requiring manual config edits.
