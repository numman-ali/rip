...
- Status (2026-04-19):
  ...
  - Implemented: canonical hosted `web_search` is now first-class through the same provider-boundary compatibility layer. OpenAI/native routes receive schema-backed `tools: [{type:"web_search", ...}]`, OpenRouter canonical requests degrade with explicit `rip.compat.warning` because its current web-search path is provider-specific (`openrouter:web_search`), and CLI/TUI/server/remote/SDK surfaces now expose the request family or a tracked surface-specific subset.
  - This is intentionally the start of a broader provider/model health layer, not the end state.
- Ready:
  - Add provider-extension hosted tools under `openresponses.extensions`, starting with OpenRouter `openrouter:web_search`, with request/stream fixtures and no silent translation from canonical `web_search`.
  - Add richer TUI and CLI/SDK controls for web-search `user_location`, future schema-backed `filters`, and route-specific model-picker chips.
