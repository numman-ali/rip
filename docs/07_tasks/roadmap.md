...
- Ready:
  ...
  - Add more curated request/runtime health: request fields (`store`, `background`, `service_tier`, include defaults/per-value subsets), tool/runtime quirks, stateful vs stateless expectations, multimodal/image support, and response-shape deltas.
  ...
  - Lift the remaining documented-only request knobs into the real runtime/config surface where appropriate: multimodal/image/file inputs, hosted tools, MCP request composition, and finer-grained per-value include/provider-model subsets where route evidence exists.
