...
Phase 1 — Foundation
- Continuities & runs (sessions): continuity-backed “one chat” state + session runner + event log + server API + CLI (fullscreen `rip` + headless); hook: session lifecycle events.
