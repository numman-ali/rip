...
pub(super) fn render_help_overlay(
    ...
    lines.push(Line::from(Span::styled(
        truncate(
            "Esc stops the active run when no overlay owns input. Ctrl-C quits and, by default, stops the active run first. Use Command Palette → Detach and keep running when you want to leave a run alive.",
            width.saturating_sub(2),
        ),
        theme.muted,
    )));
